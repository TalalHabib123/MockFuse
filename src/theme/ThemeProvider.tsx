...
import { settingsGet, settingsSetThemePreference } from "../core/settings";
...
export function ThemeProvider({ children }: { children: React.ReactNode }) {
  ...
  // 3) On app start: load persisted settings from Rust and sync
  useEffect(() => {
    let cancelled = false;
    ...
    (async () => {
      try {
        ...
        const persisted = s.themePreference; // "system" | "light" | "dark"
        if (cancelled) return;
        ...
        // If settings aren't ready yet, we just keep localStorage value.
        console.warn("[Theme] settings_get failed, using cached theme.", e);
      }
    })();
